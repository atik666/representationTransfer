...
from MiniImagenet_sup import MiniImagenet
import argparse
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Few-shot RN for supervised representation-transfer learning."
        )
    ...
    if args.transfer == True:
        try:
            net.load_state_dict(torch.load('ckpt/compare%d%d_unsup.mdl'%(args.n_way, args.k_shot)))
    ...
    mini = MiniImagenet(args.path, mode='train', n_way=args.n_way, k_shot=args.k_shot, k_query=args.k_query,
		                    batchsz=args.batchsz_train, resize=args.size)
    
    mini_val = MiniImagenet(args.path, mode='test', n_way=args.n_way, k_shot=args.k_shot, k_query=args.k_query,
