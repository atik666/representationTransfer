...
class OmniglotNShot:
    ...
    def __init__(self, root, batchsz, n_way, k_shot, k_query, imgsz):
        ...
        self.transform_query = transforms.Compose([lambda x: x,
                                              #transforms.RandomAffine(degrees=[-6,6]),
                                              #transforms.Pad(random.randint(1, 5),random.randint(1, 5)),
    ...
    def load_data_cache_train(self, data_pack):
        ...
        for sample in range(10):  # num of episodes
            ...
            for i in range(self.batchsz):  # one batch means one set
                ...
                # print(np.array(x_qry).shape) # (5, 1, 1, 28, 28)
                
                x_spt = np.array(x_spt).reshape(self.n_way * self.k_shot, 1, self.resize, self.resize)
                y_spt = np.array(y_spt).reshape(self.n_way * self.k_shot)
                x_qry = np.array(x_qry).reshape(self.n_way * self.k_query, 1, self.resize, self.resize) # (5,1,28,28)
